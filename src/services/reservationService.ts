...
export class ReservationService {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      for (const item of data.items) {
        ...
        if (!restaurantItem?.is_available || restaurantItem.stock < item.quantity) {
          throw new Error(`L'item ${restaurantItem?.item.name} n'est pas disponible`);
        }
      }
      ...
      return this.mapToDetails(reservation);
    });
  }
  ...
  // GET BY USER ID 
    ...
    return reservations.map(this.mapToDetails);
  }
  ...
  // PATCH / UPDATE
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (data && !isOwner) {
        throw new ForbiddenError('Seul le client peut modifier les items');
      }
      ...
      const updated = await tx.reservation.update({
        ...
        data: {
          status: data.status,
          special_requests: data.specialRequests,
          // Ici vous pourriez ajouter la logique pour mettre à jour les items si nécessaire
        },
        include: {
          user: true,
          restaurant: true,
...
async cancel(id: number, userId: number, reason?: string): Promise<ReservationDetails> {
    return await prisma.$transaction(async (tx) => {
        ...
        // Vérification des 24h via booléen
        ...
        const updated = await tx.reservation.update({
            ...
            data: {
                status: 'CANCELED',
                special_requests: reason ? `[ANNULÉ] ${reason}` : null,
                timeslot: { update: { status: 'AVAILABLE' } }
            },
            include: { /* vos relations */ },
        });
  ...
  private mapToDetails(reservation: any): ReservationDetails {
    return {
      id: reservation.id,
