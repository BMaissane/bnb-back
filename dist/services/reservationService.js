...
exports.ReservationService = {
    async create(data) {
        return await prisma.$transaction(async (tx) => {
            ...
            if (!timeslot || timeslot.status !== 'AVAILABLE') {
                throw new errors_1.BusinessRuleError("Ce créneau n'est plus disponible");
            }
            ...
            for (const item of data.items) {
                ...
                if (!restaurantItem?.is_available || restaurantItem.stock < item.quantity) {
                    throw new errors_1.BusinessRuleError(`L'item ${restaurantItem?.item.name} n'est pas disponible`);
                }
            }
    ...
    async getAllRestaurantReservations(restaurantId) {
    ...
    async cancel(id, reason) {
        return await prisma.$transaction(async (tx) => {
            ...
            if (new Date() > deadline) {
                throw new errors_1.BusinessRuleError('Annulation impossible <24h avant');
            }
            ...
            const [updated] = await Promise.all([
                ...
                ...reservation.reservation_has_item.map(item => tx.restaurant_has_item.updateMany({
                    where: {
